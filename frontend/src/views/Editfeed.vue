  ...
  <div class="news">
    <div id="linksetting" class="allpage">
      <div class="centerpage centerlinkform">
        <input id="linkinput" readonly type="text" v-model="link">
    ...
    <div v-show="data" class="adddetails">
      ...
      <div class="setting" v-show="datatosend.path">
        ...
        <div class="setting" v-show="datatosend.path">
          <label>Feed visible by users</label> <input type="checkbox" id="state" v-model="datatosend.state">
        </div>
      </div>


        <div class="onglet" v-show="onglet=='item'">
        <div class="setting" v-show="datatosend.itemspath">
    ...
    <div class="submit">
      <button v-on:click="save()" class="submitbtn">Save change</button>
    </div>
    </div>
...
export default {
  mounted(){ 
    ...
    let id = this.$route.params.id;
        axios.get(`${env.apiUrl}/feeds/feed/${id}`,)
        .then(async function (response) {
          const data = response.data.data.data;
          self.feedid = response.data.data.id;

          if (!data.link) {
            alert("Error : can't get feed's link")
          }else{
            ...
            await self.getData();
              ...
              var value = data[key];
              self.datatosend[key] = value
              if (value) {
                  await self.setElem(key, value);
                  if (key === "itemspath") {
  ...
  methods: {
    setElem(id, value){
      ...
      if (elem) {
        if (id != "icon" && id != "itemimagepath") {
          elem.value = value;
        }else{
          elem.placeholder = "Value already set";
        }
        if (id != "state") {
          elem?.classList.add("set");
          elem.nextSibling?.classList.add("set");
          elem.previousSibling?.classList.add("set"); 
        }
        return true;
      }else{
        return false;
    ...
    save(){
      if(this.canupdate()){
        ...
        let id = this.feedid
        axios.put(`${env.apiUrl}/feeds/${id}`,data) 
    ...
    async test(inn){ //Tests and sets variable if possible
      if(inn === "path"){ //main path
        ...
        if(typeof res == "object"){
          ...
          this.showimage(false);
          document.getElementById(inn).classList.add("changed");
          document.getElementById(inn).nextSibling.classList.add("changed");
        }else{
        ...
        if(typeof res == "object"){
          ...
          document.getElementById(inn).classList.add("changed");
          document.getElementById(inn).nextSibling.classList.add("changed");
          document.getElementById(inn).previousSibling.classList.add("changed");
        }else{
